...
- All 50 assets (35 videos + 15 photos) stamped `pending` → `assumed` (7/29) → **`granted`** (8/1 consensus) — machine-readable in `manifest.json`, history preserved per-asset. Assumed ≠ explicit: explicit yeses are still collected person-by-person, and any person can overrule the assumption instantly.
